...
from anchor.models import CorpusModel, CorpusSelectionModel, DictionaryTableModel, TextFilterQuery, SpeakerModel
from montreal_forced_aligner.data import CtmInterval, WorkflowType
from montreal_forced_aligner.db import Utterance, Speaker, File, PhoneInterval, WordInterval, CorpusWorkflow
from PySide6 import QtGui, QtCore, QtWidgets, QtMultimedia, QtSvg, QtSvgWidgets
from sqlalchemy.orm import joinedload, selectinload, load_only, subqueryload
import sqlalchemy
...
class ScatterPlot(pg.ScatterPlotItem):
    ...
    def mouseDragEvent(self, ev):
        if ev.modifiers() in [QtCore.Qt.KeyboardModifier.ControlModifier, QtCore.Qt.KeyboardModifier.ShiftModifier] and ev.button() in [QtCore.Qt.MouseButton.LeftButton, QtCore.Qt.MouseButton.MiddleButton]:
            ev.accept()
            if ev.isFinish():
                self.selection_area.hide()
            else:
    ...
    def mouseClickEvent(self, ev):
        if ev.button() == QtCore.Qt.MouseButton.LeftButton or ev.button() == QtCore.Qt.MouseButton.RightButton:
            pts = self.pointsAt(ev.pos())
            if len(pts) > 0:
                self.ptsClicked = pts
                ev.accept()
                if ev.modifiers() in [QtCore.Qt.KeyboardModifier.ControlModifier, QtCore.Qt.KeyboardModifier.ShiftModifier]:
...
class UtteranceClusterView(pg.PlotWidget):
    ...
    def __init__(self, *args):
        ...
        self.legend_item = ClusterLegend(offset=(10,10), sampleType=ClusterLegendItem,
        ...
        self.highlight_pen = pg.mkPen(
            self.settings.value(self.settings.MAIN_TEXT_COLOR), width=3)
        self.hover_pen = pg.mkPen(
            self.settings.value(self.settings.ACCENT_LIGHT_COLOR), width=3)
    ...
    def set_models(self,corpus_model: CorpusModel, selection_model: CorpusSelectionModel,
    ...
    def update_point(self, sender, spots, ev:pg.GraphicsScene.mouseEvents.MouseClickEvent):
        ...
        if ev.button() == QtCore.Qt.MouseButton.LeftButton:
            ...
            self.selection_model.set_current_file(utterance.file_id, utterance.begin, utterance.end, utterance.channel, force_update=True)
        else:
    ...
    def update_plot(self):
        ...
        brushes = [self.brushes[x] for x in self.speaker_model.cluster_labels]
        self.scatter_item.setData(pos=self.speaker_model.mds, size=10,
                                  brush=brushes, hoverPen=self.hover_pen, hoverable=True)
...
class UtteranceView(QtWidgets.QWidget):
    ...
    def __init__(self, *args):
        ...
        self.tier_scroll_area.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
    ...
    def finalize_loading_utterances(self, results):
        ...
        for u in utterances:
            if u.speaker_id not in self.speaker_tiers:
                tier = SpeakerTier(self.bottom_point, self.separator_point, u.speaker, search_term=self.search_term)
        ...
        for i, (key, tier) in enumerate(self.speaker_tiers.items()):
            ...
            tier_item.setRange(xRange=[self.selection_model.min_time, self.selection_model.max_time])
        ...
        if len(self.speaker_tiers) > 1:
            ...
            self.tier_scroll_area.verticalScrollBar().setMaximum(len(self.speaker_tiers)*row_height)
            self.tier_scroll_area.setVerticalScrollBarPolicy(QtCore.Qt.ScrollBarPolicy.ScrollBarAlwaysOn)
            self.audio_layout.centralWidget.layout.setContentsMargins(0,0,self.settings.scroll_bar_height,0)
        else:
            self.audio_layout.centralWidget.layout.setContentsMargins(0,0,0,0)
            self.tier_scroll_area.setVerticalScrollBarPolicy(QtCore.Qt.ScrollBarPolicy.ScrollBarAlwaysOff)

    def finalize_loading_wave_form(self, results):
        y, file_path = results
        if self.selection_model.current_file is None or file_path != self.selection_model.current_file.sound_file.sound_file_path:
            return
    ...
    def finalize_loading_pitch_track(self, results):
        ...
        if pitch_track is None:
            return
        x = np.linspace(start=self.selection_model.min_time, stop=self.selection_model.max_time, num=pitch_track.shape[0])
    ...
    def finalize_loading_auto_wave_form(self, results):
        ...
        if begin != self.selection_model.min_time or end != self.selection_model.max_time:
            return
        x = np.linspace(start=self.selection_model.min_time, stop=self.selection_model.max_time, num=y.shape[0])
    ...
    def get_utterances(self):
        ...
        if self.selection_model.current_file is None:
            return
        self.speaker_tier_worker.stop()
        self.speaker_tier_worker.set_params(self.corpus_model.session, self.selection_model.current_file.id)
        self.speaker_tier_worker.start()

    def set_extra_tiers(self):

        workflows = self.corpus_model.session.query(CorpusWorkflow).order_by(CorpusWorkflow.time_stamp).all()
        self.extra_tiers = {}
        for w in workflows:
    ...
    def set_up_new_file(self, *args):
        ...
        self.get_utterances()
        self.waveform_worker.stop()
        self.waveform_worker.set_params(self.selection_model.current_file.sound_file.sound_file_path)
    ...
    def draw_text_grid(self):
        ...
        for i, (key, tier) in enumerate(self.speaker_tiers.items()):
            ...
            self.speaker_tier_items[key].setRange(xRange=[self.selection_model.min_time, self.selection_model.max_time])
    ...
    def get_latest_waveform(self):
        ...
        self.audio_plot.spectrogram.hide()
        self.audio_plot.pitch_track.hide()
        begin_samp = int(self.selection_model.min_time * self.selection_model.current_file.sample_rate)
        end_samp = int(self.selection_model.max_time * self.selection_model.current_file.sample_rate)
        if len(self.audio_plot.wave_form.y.shape) > 1:
            y = self.audio_plot.wave_form.y[begin_samp:end_samp, self.selection_model.selected_channel]
        else:
            y = self.audio_plot.wave_form.y[begin_samp:end_samp]
        self.spectrogram_worker.stop()
        self.spectrogram_worker.set_params(y,
        ...
        if self.selection_model.max_time - self.selection_model.min_time <= 10:
            self.pitch_track_worker.stop()
            self.pitch_track_worker.set_params(y,
                                          self.selection_model.current_file.sound_file.sample_rate,
                                       self.selection_model.min_time, self.selection_model.max_time,
                                       self.selection_model.selected_channel,
        ...
        self.auto_waveform_worker.stop()
        self.auto_waveform_worker.set_params(y,
        ...
        self.audio_plot_item.setRange(xRange=[self.selection_model.min_time, self.selection_model.max_time])
...
class UtteranceLine(pg.InfiniteLine):
    ...
    def __init__(self, *args, movingPen=None,view_min=None, view_max=None, initial=True, **kwargs):
    ...
    def hoverEvent(self, ev):
        if (not ev.isExit()) and self.movable and \
    ...
    def mouseDragEvent(self, ev):
        if self.movable and ev.button() == QtCore.Qt.MouseButton.LeftButton:
            if ev.isStart() and ((self.initial and self.pos().x() - self.mapToParent(ev.buttonDownPos()).x() < 0)
                or (not self.initial and self.pos().x() - self.mapToParent(ev.buttonDownPos()).x() > 0)):
                self.moving = True
                self._boundingRect = None
...
class UtterancePGTextItem(pg.TextItem):
    def __init__(self, item:Utterance, selection_model:CorpusSelectionModel, top_point=None, bottom_point=None, per_tier_range=None,
                 color=None, font=None, html=None, anchor=(0, 0),
                 border=None, fill=None, dictionary_model:Optional[DictionaryTableModel]=None, speaker_id:int=0):
        self.anchor = pg.Point(anchor)
        self.rotateAxis = None
    ...
    def update_times(self, begin, end):
        ...
        if (self.view_min <= self.begin < self.view_max or self.view_max >= self.end > self.view_min
    ...
    def boundingRect(self):
        ...
        self.setPos(visible_begin + x_margin,
                         self.top_point - y_margin)
...
class PhonePGTextItem(pg.TextItem):
    def __init__(self, text:str='', color=None, font=None, html=None, anchor=(0, 0),
                 border=None, fill=None, phones=None):
        ...
        for word_object in re.finditer(self.WORDS, text):
            ...
            if sb == word_object.group() and sb != sa:
                self.setFormat(word_object.start(),
                    word_object.end() - word_object.start(), self.highlight_format)
                current_align_ind += 1
...
class IntervalTextRegion(pg.GraphicsObject):
    ...
    def __init__(self, interval: CtmInterval, color, top_point, height, font=None, border=None, background_brush=None,
                 hover_brush=None, selected_brush=None, dictionary_model=None, speaker_id = None):
        ...
        self.text.setPos((self.interval.begin + self.interval.end) / 2, self.top_point - (self.height / 2))
        self.begin_line = pg.InfiniteLine()
        self.rect = QtCore.QRectF(left=self.interval.begin, top=self.top_point,
...
class TranscriberTextRegion(IntervalTextRegion):
    ...
    def __init__(self, interval: CtmInterval, color, top_point, height, font=None, border=None, background_brush=None,
                 hover_brush=None, selected_brush=None, alignment=None):
        super().__init__(interval, color,top_point,height,font,border,background_brush, hover_brush, selected_brush)
...
class Highlighter(QtGui.QSyntaxHighlighter):
    ...
    def __init__(self, *args):
        ...
        self.spellcheck_format.setUnderlineStyle(QtGui.QTextCharFormat.UnderlineStyle.SingleUnderline)
...
class MfaRegion(pg.LinearRegionItem):
    ...
    def __init__(self, item: CtmInterval, corpus_model:CorpusModel, dictionary_model: typing.Optional[DictionaryTableModel],
        ...
        self.lines = [
            UtteranceLine(QtCore.QPointF(self.item_min, 0), angle=90, initial=True,
                          view_min=self.selection_model.min_time, view_max=self.selection_model.max_time, **lineKwds),
            UtteranceLine(QtCore.QPointF(self.item_max, 0), angle=90, initial=False,
                          view_min=self.selection_model.min_time, view_max=self.selection_model.max_time, **lineKwds)]
...
class AlignmentRegion(MfaRegion):
    ...
    def __init__(self, phone_interval:CtmInterval, corpus_model:CorpusModel,
                 selection_model:CorpusSelectionModel, selected:bool=False,
                 bottom_point:float=0, top_point:float = 1):
        super().__init__(phone_interval, corpus_model, None, selection_model, selected, bottom_point, top_point)
        self.original_text = self.item.label

        self.text = pg.TextItem(self.item.label, anchor=(0.5, 0.5),
                                color=self.text_color, border=pg.mkColor('r')
                                        )
    ...
    def boundingRect(self):
        ...
        if visible_duration != self.cached_visible_duration:
            ...
            self.text.setPos(visible_begin + (visible_duration/2),
                             self.top_point - (self.per_tier_range/2))
...
class UtteranceRegion(MfaRegion):
    def __init__(self, utterance: Utterance, corpus_model:CorpusModel, dictionary_model: DictionaryTableModel,
        ...
        super().__init__(utterance, corpus_model, dictionary_model, selection_model, selected, bottom_point, top_point)
        self.item = utterance
        self.selection_model = selection_model
        ...
        self.text = UtterancePGTextItem(self.item, self.selection_model, anchor=(0, 0),
                                        top_point=self.top_point, bottom_point=self.bottom_point, per_tier_range=self.per_tier_range,
                                        dictionary_model = self.dictionary_model,
                                        font = self.settings.font, speaker_id=self.item.speaker_id,
                                color=self.text_color, border=pg.mkPen(self.settings.accent_light_color)
                                        )
        self.text.setFont(self.plot_text_font)
        self.text.setParentItem(self)
        self.speaker_dropdown = UtteranceSpeakerDropDownItem(self.item, self.corpus_model,
                                                             font=self.settings.small_font, anchor=(0,1))
        ...
        self.text_edit.setViewportMargins(self.text_margin_pixels,self.text_margin_pixels,
                                          self.text_margin_pixels,self.text_margin_pixels)
        self.text_edit.setStyleSheet(self.settings.generate_interval_style_sheet())
        self.speaker_dropdown.combo_box.setStyleSheet(self.settings.generate_combo_box_style_sheet())
        ...
        for i, (tier_name, lookup) in enumerate(self.extra_tiers.items()):
            ...
            if lookup == 'transcription_text':
                if self.item.text and self.item.transcription_text:
                    alignment = pairwise2.align.globalms(
                        self.item.text.split(), self.item.transcription_text.split(),
                        0, -2, -1, -1, gap_char=["-"], one_alignment_only=True
                    )[0]
                intervals = [CtmInterval(self.item.begin, self.item.end, self.item.transcription_text)]
            ...
            for interval in intervals:
                ...
                if lookup == 'transcription_text':
                    interval_reg = TranscriberTextRegion(interval, self.text_color,
                                                         border=pg.mkPen(self.settings.accent_light_color),
                                                         top_point=tier_top_point, height=self.per_tier_range,
                                                                               font=self.settings.font,
                                                  alignment=alignment,
                                                  background_brush=self.background_brush, selected_brush=pg.mkBrush(self.selected_range_color))
                elif 'phone_intervals' in lookup:
                    interval_reg = PhoneRegion(interval, self.corpus_model,
                                               selection_model=selection_model,selected=False,
                                               top_point=tier_top_point, bottom_point=tier_bottom_point)
                elif 'word_intervals' in lookup:
                    interval_reg = WordRegion(interval, self.corpus_model,
                                               selection_model=selection_model,selected=False,
                                               top_point=tier_top_point, bottom_point=tier_bottom_point)

                else:
                    interval_reg = IntervalTextRegion(interval, self.text_color, border=pg.mkPen(self.settings.accent_light_color, width=3),
                                                                               top_point=tier_top_point, height=self.per_tier_range,
                                                                               font=self.settings.font,
    ...
    def eventFilter(self, obj, event):
        if event.type() == QtCore.QEvent.Type.KeyPress:
            ...
            redo_combo = QtCore.QKeyCombination(QtCore.Qt.Modifier.CTRL | QtCore.Qt.Modifier.SHIFT, QtCore.Qt.Key.Key_Z)
            if key_event.key() == QtCore.Qt.Key.Key_Tab:
                self.playRequested.emit()
                return True
            if key_event.keyCombination() == undo_combo and not self.text_edit.document().isUndoAvailable():
                self.undoRequested.emit()
                return True
            if key_event.keyCombination() == redo_combo and not self.text_edit.document().isRedoAvailable():
    ...
    def update_text_from_model(self, utterance_id, new_text):
        ...
        with QtCore.QSignalBlocker(self.text.text_edit) as blocker:
...
class PitchTrack(pg.PlotCurveItem):
    def __init__(self, bottom_point, top_point):
        ...
        self.selection_model = None
        self.setAcceptHoverEvents(False)
        self.min_label = pg.TextItem(str(self.settings.PITCH_MIN_F0),
        ...
        self.max_label = pg.TextItem(str(self.settings.PITCH_MAX_F0),
                                     self.settings.value(self.settings.PRIMARY_VERY_LIGHT_COLOR),
                                     anchor=(1,0))
...
class Spectrogram(pg.ImageItem):
    def __init__(self, bottom_point, top_point):
        ...
        self.cmap = pg.ColorMap(None, [self.settings.primary_very_dark_color, self.settings.accent_light_color])
        self.cmap.linearize()
    ...
    def setData(self, stft, channel, begin, end, min_db, max_db):
        ...
        self.setImage(self.stft, colorMap=self.cmap,
                      rect=rect
                      )
        self.show()
...
class SelectionArea(pg.LinearRegionItem):
    def __init__(self, top_point, bottom_point,
        ...
        self.settings = AnchorSettings()
        self.selection_model: typing.Optional[CorpusSelectionModel] = None
        super(SelectionArea, self).__init__(values=(-10,-5), span=(bottom_point/top_point, 1),
                                                brush= brush,
                                                  movable=False,
                                                  #clipItem=clipItem,
                                                  pen=pen,
        ...
        self.setZValue(30)
        self.lines[0].label = pg.InfLineLabel(self.lines[0], text="",
                                              position = 1, anchors=[(1,0), (1,0)]
                                              )
        self.lines[1].label = pg.InfLineLabel(self.lines[1], text="",
                                              position = 1, anchors=[(0,0), (0,0)])
        font = self.settings.font
        font.setBold(True)
    ...
    def update_region(self):
        ...
        if begin is None or end is None or (begin == self.selection_model.min_time and end == self.selection_model.max_time):
...
class AudioPlots(pg.GraphicsObject):
    def __init__(self, top_point, separator_point, bottom_point):
        ...
        self.selection_area = SelectionArea(top_point= self.top_point, bottom_point=self.bottom_point,
                                            brush=self.selection_brush, clipItem=self,
        ...
        self.picture = QtGui.QPicture()
        self.rect = QtCore.QRectF(left=0, top=self.top_point, width=10,
                                  height=self.top_point -self.bottom_point)
        self.rect.setTop(self.top_point)
    ...
    def update_plot(self):
        if self.selection_model.current_file is None or self.selection_model.current_file.sound_file is None or not os.path.exists(self.selection_model.current_file.sound_file.sound_file_path):
            return
...
class SpeakerTier(pg.GraphicsObject):
    ...
    def mouseDoubleClickEvent(self, ev):
        ...
        for x in self.visible_utterances.values():
            ...
            if end > x.item_min and end < x.item_min:
                ...
                break
        if end - begin > 0.001:
            self.corpus_model.create_utterance(self.selection_model.current_file, self.speaker,
                                           begin, end)
            ev.accept()
    ...
    def set_speaker_index(self, index, num_speakers):
        ...
        self.rect = QtCore.QRectF(left=self.selection_model.min_time, top=self.top_point,
    ...
    def set_models(self, corpus_model: CorpusModel, selection_model:CorpusSelectionModel, dictionary_model:DictionaryTableModel):
        self.corpus_model = corpus_model
        self.selection_model = selection_model
        self.dictionary_model = dictionary_model
        for reg in self.visible_utterances.values():
    ...
    def refresh(self, *args):
        ...
        for u in self.utterances:
            ...
            reg =  UtteranceRegion(u, self.corpus_model,
                                   self.dictionary_model,
                                   selection_model=self.selection_model,
                                          selected=selected, extra_tiers=self.extra_tiers, available_speakers=self.available_speakers,
                                          bottom_point=self.bottom_point,
                                   top_point=self.top_point,  search_term=self.search_term)
