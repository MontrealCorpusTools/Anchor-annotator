...
from PySide6 import QtGui, QtCore, QtWidgets, QtMultimedia, QtSvgWidgets

import re
from anchor.models import CorpusModel, CorpusSelectionModel, DictionaryTableModel, SpeakerModel, TextFilterQuery, OovModel, MergeSpeakerModel
from anchor.settings import AnchorSettings
from anchor.workers import Worker
from anchor.plot import UtteranceView, UtteranceClusterView
from montreal_forced_aligner.db import Speaker, Phone  # noqa
from montreal_forced_aligner.data import PhoneType, PhoneSetType, DistanceMetric, ClusterType, ManifoldAlgorithm  # noqa
from montreal_forced_aligner.utils import inspect_database, DatasetType  # noqa
...
class MediaPlayer(QtMultimedia.QMediaPlayer):  # pragma: no cover
    ...
    def play(self) -> None:
        ...
        if self.playbackState() == QtMultimedia.QMediaPlayer.PlaybackState.StoppedState or \
                self.currentTime() < self.startTime() or self.currentTime() >= self.maxTime():
    ...
    def set_corpus_models(self, corpus_model: Optional[CorpusModel], selection_model: Optional[CorpusSelectionModel]):
    ...
    def set_volume(self, volume: int):
        if self.audioOutput() is None:
            return
        linearVolume = QtMultimedia.QAudio.convertVolume(volume / 100.0,
                        QtMultimedia.QAudio.VolumeScale.LogarithmicVolumeScale,
                        QtMultimedia.QAudio.VolumeScale.LinearVolumeScale)
        #self.audioOutput().setVolume(round(linearVolume * 100))
    ...
    def volume(self) -> int:
        ...
        volume = QtMultimedia.QAudio.convertVolume(volume / 100.0,
                        QtMultimedia.QAudio.VolumeScale.LinearVolumeScale,
                        QtMultimedia.QAudio.VolumeScale.LogarithmicVolumeScale)
        return int(volume)
    ...
    def update_times(self):
        if self.playbackState() == QtMultimedia.QMediaPlayer.PlaybackState.StoppedState \
    ...
    def loadNewFile(self, *args):
        ...
        try:
            ...
            return
        if self.selection_model.max_time is None or self.selection_model.current_file is None or \
                self.selection_model.current_file.duration is None:
            self.setSource(QtCore.QUrl())
            return
    ...
    def checkStop(self):
        if not self.hasAudio():
            self.stop()
            self.setSource(QtCore.QUrl.fromLocalFile(self.selection_model.current_file.sound_file.sound_file_path))
            self.play()
            return
        if self.playbackState() == QtMultimedia.QMediaPlayer.PlaybackState.PlayingState:
...
class AnchorTableView(QtWidgets.QTableView):
    ...
    def refresh_settings(self):
        ...
        for i in range(self.horizontalHeader().count()):
            text = self.model().headerData(i, QtCore.Qt.Orientation.Horizontal, QtCore.Qt.ItemDataRole.DisplayRole)
...
class PaginationWidget(QtWidgets.QToolBar):
    ...
    def __init__(self,*args):
        ...
        self.num_pages = 1
        self.result_count = 0
        self.next_page_action = QtGui.QAction(icon=QtGui.QIcon(':caret-right.svg'), text='Next page')
        self.previous_page_action = QtGui.QAction(icon=QtGui.QIcon(':caret-left.svg'), text='Previous page')
...
class UtteranceListWidget(QtWidgets.QWidget):  # pragma: no cover
    ...
    def set_models(self, corpus_model:CorpusModel, selection_model:CorpusSelectionModel, speaker_model: SpeakerModel):
    ...
    def search(self):
        ...
        self.corpus_model.search(self.search_box.query(), self.file_dropdown.current_text(), self.speaker_dropdown.current_text(), oovs=self.oov_button.isChecked())
...
class UtteranceDetailWidget(QtWidgets.QWidget):  # pragma: no cover
    ...
    def set_models(self, corpus_model: CorpusModel,
                   selection_model: CorpusSelectionModel, dictionary_model: DictionaryTableModel):
        self.corpus_model = corpus_model
        self.selection_model = selection_model
    ...
    def update_to_slider(self):
        with QtCore.QSignalBlocker(self.scroll_bar):
            if self.selection_model.current_file is None or self.selection_model.min_time is None:
                return
            if self.selection_model.min_time == 0 and self.selection_model.max_time == self.selection_model.current_file.duration:
...
class SearchBox(ClearableField):
    ...
    def __init__(self, *args):
        ...
        regex_icon.addFile(':highlighted/regex.svg', mode=QtGui.QIcon.Mode.Normal, state=QtGui.QIcon.State.On)
        ...
        word_icon.addFile(':highlighted/word.svg', mode=QtGui.QIcon.Mode.Normal, state=QtGui.QIcon.State.On)
        ...
        case_icon.addFile(':highlighted/case.svg', mode=QtGui.QIcon.Mode.Normal, state=QtGui.QIcon.State.On)
    ...
    def setQuery(self, query: TextFilterQuery):
        ...
        with QtCore.QSignalBlocker(self.regex_action) as _, QtCore.QSignalBlocker(self.word_action) as _:
    ...
    def query(self) -> TextFilterQuery:
        filter = TextFilterQuery(super().text(),
...
class NoWrapDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        option.palette.setColor(QtGui.QPalette.ColorGroup.Active, QtGui.QPalette.ColorRole.Window, QtGui.QColor(selection_color))
        painter.save()
        options = QtWidgets.QStyleOptionViewItem(option)
        self.initStyleOption(options, index)
        self.doc.setPlainText(options.text)
...
class HighlightDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        option.palette.setColor(QtGui.QPalette.ColorGroup.Active, QtGui.QPalette.ColorRole.Window, selection_color)
        painter.save()
        options = QtWidgets.QStyleOptionViewItem(option)
        self.initStyleOption(options, index)
        self.doc.setPlainText(options.text)
...
class HeaderView(QtWidgets.QHeaderView):
    ...
    def generate_context_menu(self, location):
        ...
        for i in range(m.columnCount()):
            column_name = m.headerData(i, orientation=QtCore.Qt.Orientation.Horizontal, role=QtCore.Qt.ItemDataRole.DisplayRole)
...
class ProgressMenu(QtWidgets.QMenu):
    ...
    def __init__(self, *args):
        ...
        self.scroll_area.setFixedWidth(500+ self.scroll_area.verticalScrollBar().sizeHint().width())
    ...
    def showEvent(self, event:QtGui.QShowEvent) -> None:
        ...
        self.move(p.x()+geo.width()-self.geometry().width(), p.y()-geo.height()-self.geometry().height())
...
class SpeakerClusterSettingsMenu(QtWidgets.QMenu):
    def __init__(self, *args):
        ...
        self.row_indices = {}
        self.cluster_algorithm_dropdown.setCurrentIndex(self.cluster_algorithm_dropdown.findText(self.settings.value(self.settings.CLUSTER_TYPE)))
        self.metric_dropdown.setCurrentIndex(self.metric_dropdown.findText(self.settings.value(self.settings.CLUSTERING_METRIC)))
        ...
        self.distance_threshold_edit.setValue(self.settings.value(self.settings.CLUSTERING_DISTANCE_THRESHOLD))
        self.min_cluster_size_edit.setValue(self.settings.value(self.settings.CLUSTERING_MIN_CLUSTER_SIZE))
        ...
        self.cluster_algorithm_dropdown.currentIndexChanged.connect(self.update_current_cluster_algorithm)
    ...
    def cluster_kwargs(self):
        ...
        if current_algorithm in [ClusterType.kmeans, ClusterType.spectral, ClusterType.agglomerative]:
    ...
    def manifold_kwargs(self):
        kwargs = {
            'metric': DistanceMetric[self.metric_dropdown.currentText()]
        }
        return kwargs
...
class IpaKeyboard(QtWidgets.QMenu):
    ...
    def __init__(self, phones, parent=None):
        ...
        column_count = 10
        self.buttons = [
            QtWidgets.QPushButton(p) for p in sorted(phones)
        ]
        col_index = 0
        row_index = 0
        ...
        self.setLayout(layout)
        self.setFocusPolicy(QtCore.Qt.FocusPolicy.NoFocus)
        self.scroll_area.setMinimumWidth(widget.sizeHint().width()+ self.scroll_area.verticalScrollBar().sizeHint().width())
        self.scroll_area.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
    ...
    def highlightBlock(self, text):
        for phone_object in re.finditer(self.PHONES, text):
            ...
            if phone_object.group() not in self.phones:
                self.setFormat(phone_object.start(),
                    phone_object.end() - phone_object.start(), self.highlight_format)
...
class PronunciationInput(QtWidgets.QToolBar):
    ...
    def __init__(self, phones, *args, icon_size=25):
        ...
        accept_icon.addFile(':check-circle.svg', mode=QtGui.QIcon.Mode.Normal, state=QtGui.QIcon.State.Off)
        accept_icon.addFile(':highlighted/check-circle.svg', mode=QtGui.QIcon.Mode.Normal, state=QtGui.QIcon.State.On)
        ...
        cancel_icon.addFile(':highlighted/undo.svg', mode=QtGui.QIcon.Mode.Normal, state=QtGui.QIcon.State.On)
        ...
        keyboard_icon.addFile(':keyboard.svg', mode=QtGui.QIcon.Mode.Normal, state=QtGui.QIcon.State.Off)
        keyboard_icon.addFile(':highlighted/keyboard.svg', mode=QtGui.QIcon.Mode.Normal, state=QtGui.QIcon.State.On)
    ...
    def eventFilter(self, watched:QtCore.QObject, event:QtCore.QEvent) -> bool:
        if isinstance(watched, (PronunciationField, IpaKeyboard)) and event.type() == QtCore.QEvent.Type.KeyPress and \
                event.key() in {QtGui.Qt.Key.Key_Enter, QtGui.Qt.Key.Key_Return, QtGui.Qt.Key.Key_Tab}:
            if self.accept_action.isEnabled():
                self.returnPressed.emit()
            return True
        elif isinstance(watched, (IpaKeyboard)) and event.type() == QtCore.QEvent.Type.KeyPress and \
                event.key() not in {QtGui.Qt.Key.Key_Escape}:
...
class CountDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def paint(self, painter:QtGui.QPainter,
              ...
              index:typing.Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]) -> None:
...
class EditableDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def createEditor(self, parent:DictionaryTableView, option:QtWidgets.QStyleOptionViewItem,
                     index:typing.Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]) -> QtWidgets.QWidget:
    ...
    def setEditorData(self, editor:PronunciationInput,
                      index:typing.Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]) -> None:
        editor.setText(index.model().data(index, QtCore.Qt.ItemDataRole.EditRole))

    def setModelData(self, editor:PronunciationInput,
    ...
    def updateEditorGeometry(self, editor:PronunciationInput,
                             option:QtWidgets.QStyleOptionViewItem,
                             index:typing.Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]) -> None:
...
class PronunciationDelegate(EditableDelegate):
    ...
    def eventFilter(self, object:QtCore.QObject, event:QtCore.QEvent) -> bool:
        if event.type() == QtCore.QEvent.Type.KeyPress:
            if isinstance(object, PronunciationInput) or isinstance(object.parent(), PronunciationInput):
                if isinstance(object.parent(), PronunciationInput):
                    object = object.parent()
                if event.key() in {QtGui.Qt.Key.Key_Enter, QtGui.Qt.Key.Key_Return, QtGui.Qt.Key.Key_Tab}:
                    self.commitData.emit(object)
                    return True
        return super().eventFilter(object, event)

    def sizeHint(self, option:QtWidgets.QStyleOptionViewItem,
                 index:typing.Union[QtCore.QModelIndex, QtCore.QPersistentModelIndex]) -> QtCore.QSize:
...
class OovTableView(AnchorTableView):
    ...
    def add_pronunciation(self):
        ...
        if not rows:
            return
        word = self.oov_model.data(self.oov_model.createIndex(rows[0].row(), 0), QtCore.Qt.ItemDataRole.DisplayRole)
...
class DictionaryTableView(AnchorTableView):
    ...
    def add_pronunciation(self):
        ...
        word = self.dictionary_model.data(self.dictionary_model.createIndex(rows[0].row(), 0), QtCore.Qt.ItemDataRole.DisplayRole)
    ...
    def calculate_spans(self):
        for i in range(self.dictionary_model.rowCount()):
            ...
            if i > 0 and self.dictionary_model.word_indices[i-1] == self.dictionary_model.word_indices[i]:
...
class SpeakerTableView(AnchorTableView):
    ...
    def search_speaker(self, index:QtCore.QModelIndex):
        if not index.isValid() or index.column() != 1:
            return
        speaker = self.model().data(self.model().index(index.row(), 0), QtCore.Qt.ItemDataRole.DisplayRole)
...
class TranscriberWidget(QtWidgets.QWidget):
    ...
    def refresh(self):
        ...
        if dataset_type in {DatasetType.ACOUSTIC_CORPUS_WITH_DICTIONARY,
...
class SpeakerClustersWidget(QtWidgets.QWidget):
    ...
    def recluster(self):
        self.speaker_model.update_manifold_kwargs(self.cluster_settings_widget.menu.manifold_kwargs)
...
class SpeakerMergeTable(AnchorTableView):
    ...
    def search_speaker(self, index:QtCore.QModelIndex):
        if not index.isValid() or index.column() > 1:
            return
        speaker = self.model().data(self.model().index(index.row(), index.column()), QtCore.Qt.ItemDataRole.DisplayRole)
        self.searchRequested.emit(speaker)
...
class DiarizationWidget(QtWidgets.QWidget):
    ...
    def refresh(self):
        validate_enabled = True
        if self.merge_speaker_model.corpus_model.ivector_extractor is not None and self.merge_speaker_model.corpus_model.corpus is not None:
            ...
            self.search_action.setEnabled(self.merge_speaker_model.corpus_model.corpus.has_any_ivectors())
            self.threshold_edit.setEnabled(self.merge_speaker_model.corpus_model.corpus.has_any_ivectors())
        else:
            validate_enabled = False
    ...
    def set_models(self, model: MergeSpeakerModel):
        ...
        self.merge_speaker_model.resultCountChanged.connect(self.pagination_toolbar.update_result_count)
        ...
        self.merge_speaker_model.corpus_model.speakersRefreshed.connect(self.speaker_dropdown.update_completions)
    ...
    def update_speaker_count(self):
        self.pagination_toolbar.update_result_count(self.merge_speaker_model.corpus_model.corpus.num_speakers)
...
class DictionaryWidget(QtWidgets.QWidget):
    ...
    def dictionaries_refreshed(self, dictionaries):
        ...
        if not dictionaries:
            return
        for i, (d_id, d_name) in enumerate(dictionaries):
    ...
    def set_models(self, dictionary_model: DictionaryTableModel):
        ...
        self.dictionary_model.resultCountChanged.connect(self.pagination_toolbar.update_result_count)
...
class SpeakerWidget(QtWidgets.QWidget):
    ...
    def update_speaker_count(self):
        self.pagination_toolbar.update_result_count(self.speaker_model.corpus_model.corpus.num_speakers)
    ...
    def refresh_settings(self):
        ...
        self.search_box.setStyleSheet(self.settings.generate_search_box_style_sheet())
        self.table.refresh_settings()
        self.pagination_toolbar.set_limit(self.table.settings.value(self.table.settings.RESULTS_PER_PAGE))
